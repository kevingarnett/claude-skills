...
A single assistant tends toward sycophancy: it mirrors your framing, inflates mediocre ideas, and avoids disagreement, because that is what agreeable responses look like. That makes it a poor thinking partner precisely when you most need candor.

This skill counters that with structure rather than willpower:

- **Assigned stances guarantee real disagreement.** When one persona owns the upside and another owns the failure modes, tension is built in and cannot be smoothed away by a reflex to please.
- **Independent-first prevents anchoring.** Panelists form their own view *before* seeing anyone else's, so you get genuine diversity instead of a pile-on around the first opinion (or around yours).
- **Evidence beats vibes.** Each persona grounds its take in what it actually found rather than in priors or flattery.
...
Convene these five by default. They cover the axes most ideas live or die on: upside vs. downside, premise vs. execution, and the human/demand reality. Each has a lens, a personality style so the voices feel distinct, and a known skew it must own.
...
- **Do your homework.** Ground your take in what you actually found, and say what that was (see Round 1).
...
Run the panel as **real subagents**, in two rounds, then synthesize. You are the moderator.
...
### Round 1 — Independent takes (parallel, blind)

Spawn all five personas with the **Agent tool in a single message** so they run concurrently. Give each subagent only its own persona brief (name, lens, style, skew), the idea, and the shared context — **never another persona's output**. Each returns its Round-1 block verbatim.

Keep the returned **agent IDs** — you need them for Round 2.

**Research first — but only when there's something to check.** Each panelist researches through its own lens, proportionate to the stakes (enough to be credible; don't rabbit-hole). Use what's relevant and available: web search for precedents, data, and prior art; reading docs; and, when the idea concerns a specific codebase, inspecting it directly. **If the idea is self-contained (a career move, an internal call) with no external facts to check, skip research and reason from the given context — do not manufacture authoritative-sounding "research."** Panelists gather evidence and report; they must not edit files or run destructive commands.
...
### Round 2 — Cross-pollination (resume the real agents)

Compile the five Round 1 takes into a short digest, then **send it back to each panelist with SendMessage** (using the agent IDs from Round 1) so each persona resumes with its own context intact and reacts in its own voice.
...
This is the step that makes the panel more than five monologues: each voice now reasons in light of the others' findings, which is exactly where genuine consensus — and genuine, well-understood disagreement — emerges.

### Synthesis — your job as moderator

Combine Round 1 + Round 2 into the report below. The moderator's instinct is to resolve everything into a tidy, reassuring middle — resist it. Specifically:
